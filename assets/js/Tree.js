...
this.Tree = new Class({
	...
	mousedown: function(element, event){
		...
		this.clone = element.clone().setStyles({
			left: this.options.cloneOffset.x, // event.page.x + 
			top: this.current.getCoordinates().top.toInt() + 2, //event.page.y + this.options.cloneOffset.y,
			opacity: this.options.cloneOpacity,
            paddingLeft: this.current.getChildren('.name').getStyle('padding-left')[0].toInt() + 10
	...
	onDrag: function(el, event){
		...
		var drop;
        console.log([droppable, droppable.getParent('li')].contains(this.current));
